...
class TBT_Rewardsinstore_Model_Transfer_Quote extends TBT_Rewards_Model_Transfer {
    ...
    public function create($num_points, $currency_id, $customerId, $quoteId, $comment = "Points received for making an Instore purchase", $reason_id = TBT_Rewards_Model_Transfer_Reason::REASON_CUSTOMER_DISTRIBUTION, // TODO: use new reason (careful, this makes transfers not show up in grid)
            $transferStatus = TBT_Rewards_Model_Transfer_Status::STATUS_APPROVED) {
        Mage::log('rewardsinstore/transfer_quote::create() ' . __LINE__, null, 'rewards-log.txt');
        // ALWAYS ensure that we only give an integral amount of points
        $num_points = floor($num_points);
        if ($num_points <= 0) {
            Mage::log('points: ' . $num_points, null, 'rewards-log.txt');
            return $this;
        }
        ...
        if (!$this->setStatus(null, $transferStatus)) {
            ...
            Mage::log('unable to set ' . $transferStatus . " - returning", null, 'rewards-log.txt');
            return $this;
        }
        try {
            $this->setId(null)
                    ...
                    ->save();
            Mage::log($this->getData(), null, 'rewards-log.txt');
        } catch (Exception $ex) {
            Mage::log($ex, null, 'rewards-log.txt');
        }
        return $this;
    }
