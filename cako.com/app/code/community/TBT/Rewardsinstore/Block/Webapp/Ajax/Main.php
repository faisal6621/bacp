...
class TBT_Rewardsinstore_Block_Webapp_Ajax_Main extends Mage_Adminhtml_Block_Template
{
    ...
    {
        return Mage::getModel('rewardsinstore/storefront')->getActiveStorefronts();
    }
    ...
    protected function getStorefrontId()
    {
        if ($this->_storefrontId === null) {
            if (!$this->_storefrontId = $this->getStorefrontIdFromSession()) {
                $storefronts = $this->getStorefrontCollection();
                if (count($storefronts) == 1) {
                    $this->_storefrontId = $storefronts->getFirstItem()->getId();
                } else {
                    $this->_storefrontId = Mage::getModel('rewardsinstore/storefront')
                        ->load($this->getSelectedStorefront(), 'code')
                        ->getId();
                }
                Mage::getSingleton('admin/session')->setStorefrontId($this->_storefrontId);
            }
        }
        
        return $this->_storefrontId ? $this->_storefrontId : "null";
    }
    
    protected function getStorefrontIdFromSession()
    {
        $session = Mage::getSingleton('admin/session');
        return $session->hasStorefrontId() ?
            $session->getStorefrontId() :
            null;
    }
