...
class TBT_Rewards_Model_Customer extends Mage_Customer_Model_Customer {
    ...
    public function getRewardsCustomer($customer) {
        if ($customer instanceof TBT_Rewards_Model_Customer) {
            Mage::log(__FILE__ . "::" . __LINE__ . "\n" . get_class($customer), null, 'rewards-log.txt');
            return $customer;
        }
        if ($customer == null) {
        ...
        if ($customer->getId()) {
            Mage::log(__FILE__ . "::" . __LINE__ . "\n" . $customer->getId(), null, 'rewards-log.txt');
    ...
    public function createTransferForNewCustomer() {
        ...
        foreach ($ruleCollection as $rule) {
            try {
                ...
                Mage::log($ex, NULL, 'reward-points.txt');
            }
            ...
            if ($is_transfer_successful) {
                ...
                Mage::log(__FILE__ . "::" . __LINE__, null, 'reward-points.txt');
                Mage::log(Mage::helper('rewards')->__('You received %s for signing up!', (string) Mage::getModel('rewards/points')->set($rule)), null, 'reward-points.txt');
            } else {
