...
class TBT_Rewards_Model_Catalogrule_Observer extends Mage_CatalogRule_Model_Observer {
    ...
    public function updateRedemptions($o) {
        ...
        if ($cart) {
            $quote = $cart->getQuote(); //
            Mage::log('cart found. getting quote from cart', null, 'rewards-log.txt');
        } else {
            $quote = $event->getQuote(); //
            Mage::log('cart not found. getting quote from event', null, 'rewards-log.txt');
        }
    ...
    public function updateQuoteCatalogRedemptions($quote, $data = null, $date = null, $wId = null) {
        ...
        try {
            if (!$quote) {
                Mage::log('quote not found. return', null, 'rewards-log.txt');
                return $this;
            }
