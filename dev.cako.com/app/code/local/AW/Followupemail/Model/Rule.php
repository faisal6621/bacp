...
class AW_Followupemail_Model_Rule extends Mage_Core_Model_Abstract {
    ...
    public function validateByProduct($params) {
        ...
        if ($this->_sku) {
            ...
            if (count($this->_sku) && isset($params['sku']) && count($params['sku']) && !array_intersect(
        ...
        if (count($this->_productTypes) && !in_array(
                        AW_Followupemail_Model_Source_Product_Types::PRODUCT_TYPE_ALL, $this->_productTypes
                ) && isset($params['product_type_ids']) && count($params['product_type_ids']) && count($this->_sku) && isset($params['sku']) && count($params['sku'])
        ) {
    ...
    protected function _getTemplate($templateId, $storeId = null) {
        ...
        $templateName = substr(
                $templateId, false !== (
                $pos = strpos($templateId, AW_Followupemail_Model_Source_Rule_Template::TEMPLATE_SOURCE_SEPARATOR)) ? $pos + 1 : 0
        );
        ...
        if (!isset($objects['customer_name'])) {
            if (isset($objects['customer_email'])) {
                $objects['customer_name'] = substr(
                        $objects['customer_email'], 0, false !== ($pos = strpos($objects['customer_email'], '@')) ? $pos : 999
                );
            } else {
    ...
    private function _getCrossProducts($objects) {
                ...
                if (count($ids) > 0) {
                    foreach ($ids as $arp2Block) {
                        $boughtProductIds = array_merge(
                                $boughtProductIds, Mage::getModel('awautorelated/api')->getRelatedProductsForShoppingCartRule(
                                        $arp2Block, $quoteId
                                )
                        );
                    }
        ...
        $relProductCollection->addAttributeToSelect('thumbnail')
                ...
                ->addPriceData(
                        $objectForCross->getCustomerGroupId(), Mage::app()->getStore($objectForCross->getStoreId())->getWebsiteId()
                )
    ...
    public function process($params, $objects = array()) {
        ...
        if ($this->_isValid) {
            ...
            if (!($this->getChain() && count($chain = unserialize($this->getChain())))) {
                Mage::getSingleton('followupemail/log')->logWarning(
                        'rule id=' . $this->getId() . ' has no chain or the chain is empty: "' . $this->getChain() . '"', $this
                );
                return false;
            }
            if (
            ...
            foreach ($chain as $chainItem) {
                ...
                if (!$content = $this->_getContent($objects, $chainItem['TEMPLATE_ID'])) {
                    ...
                    $queue->add(
                            $code, $sequenceNumber, $content['sender_name'], $content['sender_email'], $objects['customer_name'], ($this->_isTest) ? $this->getTestRecipient() : $objects['customer_email'], $this->getId(), time() + $objects['time_delay'] * 60, ($this->_isTest) ? $testFlag . $content['subject'] : $content['subject'], ($this->_isTest) ? $testFlag . $content['content'] : $content['content'], $objects['object_id'], $params, $content['template_styles']
                    );
                }
                $sequenceNumber++;
            }
        ...
        Mage::getSingleton('followupemail/log')->logWarning(
                ...
                . $this->_validationMessage . '" objectId=' . (isset($objects['object_id']) ? $objects['object_id'] : 'none') . ', params="' . AW_Followupemail_Helper_Data::printParams($params), $this
        );
    ...
    public function processBirthday($params, $templateId, $timeDelay, $sequenceNumber) {
        ...
        if ($this->_isValid) {
            if (!(isset($params['customer_id']) && $params['customer_id'])) {
                $message = 'rule id=' . $this->getId() . ' processing error: customer_id is not set on birthday event, email="'
            ...
            $queue->add(
                    ...
                    $this->getId(), time(), $content['subject'], $content['content'], $params['object_id'], $storedParams, $content['template_styles']
            );
            return true;
        }
        Mage::getSingleton('followupemail/log')->logWarning(
