...
class AW_Followupemail_Adminhtml_RulesController extends Mage_Adminhtml_Controller_Action {
    ...
    public function editAction() {
        ...
        if (!empty($data) || $ruleId == 0) {
            ...
            if (!isset($data['customer_groups']) || !$data['customer_groups'] || !count(
                            is_array($data['customer_groups']) ? $data['customer_groups'] : ($data['customer_groups'] = explode(',', $data['customer_groups']))
                    ) || (in_array(AW_Followupemail_Model_Source_Customer_Group::CUSTOMER_GROUP_ALL, $data['customer_groups']) && count($data['customer_groups']) > 1)
            ) {
            ...
            if (Mage::helper('followupemail')->canUseAN()) {
                if (!isset($data['anl_segments']) || !$data['anl_segments'] || !count(
                                is_array($data['anl_segments']) ? $data['anl_segments'] : ($data['anl_segments'] = explode(',', $data['anl_segments']))
                        ) || (in_array(
            ...
            if (!isset($data['sale_amount_condition']) && !isset($data['sale_amount_value']) // if this is not a redirect from saveAction
                    && isset($data['sale_amount']) && $data['sale_amount'] && is_array(
                            $saleAmount = explode(
    ...
    public function saveAction($sendTest = false) {
        ...
        if ($data = $this->getRequest()->getPost()) {
            ...
            if (Mage::helper('followupemail')->canUseAN()) {
                ...
                if (!count($data['anl_segments']) || (in_array(
                                AW_Followupemail_Model_Mysql4_Rule::ADVANCED_NEWSLETTER_SEGMENTS_ALL, $data['anl_segments']
                        ) && count($data['anl_segments']) > 1)
                ) {
                    $data['anl_segments'] = array(AW_Followupemail_Model_Mysql4_Rule::ADVANCED_NEWSLETTER_SEGMENTS_ALL);
            ...
            if ($data['sale_amount_value'] || $data['sale_amount_condition']) {
                $data['sale_amount'] = AW_Followupemail_Model_Source_Rule_Saleamount::getCondition($data['sale_amount_condition'])
            ...
            if (!isset($data['chain'])) {
                ...
                foreach ($data['chain'] as $key => $value) {
                    if (false === strpos(
                                    $value['TEMPLATE_ID'], AW_Followupemail_Model_Source_Rule_Template::TEMPLATE_SOURCE_SEPARATOR
                            )
                    ) {
            ...
            if ($this->getRequest()->getParam('coupon_enabled')) {
                ...
                if (!$couponExpireDays || !intval($couponExpireDays)) {
                    ...
                    return $this->_redirect(
                                    '*/*/edit', array('id' => $this->getRequest()->getParam('id'), 'tab' => 'followupemail_tabs_coupons')
                    );
                }
