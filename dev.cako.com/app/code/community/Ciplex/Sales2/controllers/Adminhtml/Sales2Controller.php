...
class Ciplex_Sales2_Adminhtml_Sales2Controller extends Mage_Adminhtml_Controller_Action {
    ...
    protected function _construct() {
        $this->setUsedModuleName('Ciplex_Sales2');
    }
    ...
    protected function _isAllowed() {
        ...
        return Mage::getSingleton('admin/session')->isAllowed('sales/sales2backend');
    }
    ...
    public function save_fulfillAction() {

        $fulfill = $this->getRequest()->getParam('fulfill', null);
        $orderId = $this->getRequest()->getParam('order_id', null);
        $ff = implode(',', $fulfill);
        Mage::log($ff, null, "fulfill.log");
        $sales2 = Mage::getModel('sales2/sales2')->load($orderId, 'order_id');
        if ($sales2->getId()) {
            $sales2->setFulfilledBy($ff);
            Mage::log("dd", null, "fulfill.log");
        } else {
            Mage::log("ee", null, "fulfill.log");
            $sales2->setId(null);
            $sales2->setOrderId($orderId);
            $sales2->setFulfilledBy($ff);
        }
    ...
    public function save_shipping_dateAction() {

        $shippingDate = $this->getRequest()->getParam('shipping_date', null);
        $shippingTimeOld = $this->getRequest()->getParam('shipping_time', null);
        $shippingTime = '00:00:00';
        $shippingTimeH = $this->getRequest()->getParam('shipping_time_h', null);
        $shippingTimeM = $this->getRequest()->getParam('shipping_time_m', null);
        $shippingTimeA = $this->getRequest()->getParam('shipping_time_a', null);
        if (isset($shippingTimeH) && isset($shippingTimeA) && isset($shippingTimeM)) {
            if ($shippingTimeA == 'PM' && $shippingTimeH != 12) {
                $shippingTimeH += 12;
            }
            if ($shippingTimeA == 'AM' && $shippingTimeH == 12) {
                $shippingTimeH = '00';
            }
            $shippingTime = $shippingTimeH . ":" . $shippingTimeM . ":00";
        }
        ...
        $sales2 = Mage::getModel('sales2/sales2')->load($orderId, 'order_id');
        if ($sales2->getId()) {
            $sales2->setShippingDate($shippingDate);
            $sales2->setShippingTime($shippingTimeOld);
            $sales2->setShippingTime2($shippingTime);
        } else {
            $sales2->setId(null);
            $sales2->setOrderId($orderId);
            $sales2->setShippingDate($shippingDate);
    ...
    public function save_custom_orderAction() {

        $shippingDate = $this->getRequest()->getParam('custom_order', null);
        $orderId = $this->getRequest()->getParam('order_id', null);
        $sales2 = Mage::getModel('sales2/sales2')->load($orderId, 'order_id');
        if ($sales2->getId()) {
            $sales2->setCustomOrder($shippingDate);
        } else {
            $sales2->setId(null);
            $sales2->setOrderId($orderId);
            $sales2->setCustomOrder($shippingDate);
