...
//            die;
            try {
                ...
                if (isset($data['customer_id']) && $data ['transfer_style'] != 'deduct') {
                    try {
//                        $mailTemplate = Mage::getStoreConfig('customer/create_account/email_confirmed_tamplate', 'default');
                        $translate = Mage::getSingleton('core/translate');
                        $translate->setTranslateInline(false);
                        $template = Mage::getModel('core/email_template');
                        $customer = Mage::getModel('customer/customer')->load($data['customer_id']);
                        $template->setDesignConfig(array('area' => 'frontend', 'store' => 'default'))
                                ->sendTransactional(
                                        18, Mage::getStoreConfig(Mage_Sales_Model_Order::XML_PATH_UPDATE_EMAIL_IDENTITY, 'default'), $customer->getEmail(), $customer->getFirstname() . ' ' . $customer->getLastname(), array('customer' => $customer)
                        );
                        $translate->setTranslateInline(true);
                        Mage::getSingleton('adminhtml/session')->addSuccess(Mage::helper('rewards')->__('%s notified successfully at %s', $customer->getName(), $customer->getEmail()));
                    } catch (Exception $ex) {
                        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('rewards')->__($ex->getMessage()));
                    }
                }
