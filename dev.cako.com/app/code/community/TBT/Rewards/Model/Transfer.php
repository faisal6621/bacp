...
class TBT_Rewards_Model_Transfer extends TBT_Rewards_Model_Abstract {
    ...
    public function revoke() {
        ...
        if (($customer->getUsablePointsBalance($currency_id) + $num_points) < 0) {
            $error = Mage::helper('rewards')->__('Not enough points for transaction. You have %s, but you need %s', Mage::getModel('rewards/points')->set($currency_id, $customer->getUsablePointsBalance($currency_id)), Mage::getModel('rewards/points')->set($currency_id, $num_points * - 1));
            throw new Exception($error);
        }
