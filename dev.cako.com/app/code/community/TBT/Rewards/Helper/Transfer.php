...
//        echo "rule_ids_string\n";
        print_r($rule_ids_string);
                ...
                if ($item_rule->getId() == $required_id) {
                    if (Mage::helper('rewards/config')->calcCartPointsAfterDiscount()) {
                        ...
                        if ($prices_include_tax) {
                            ...
                            // ciplex modification ->if then 20121109
                            if ($item->getRowTotalAfterRedemptionsInclTax()) {
                                $price += Mage::helper('rewards/price')->getReversedCurrencyPrice($item->getRowTotalAfterRedemptionsInclTax());
                            } else {
                                $price += Mage::helper('rewards/price')->getReversedCurrencyPrice($item->getBaseRowTotal());
                            }
                        } else {
                            // we don't fall back to getBaseRowTotal here because applied redemptions can legitimately drop this value to 0
                            // ciplex modification ->if then 20121109
                            if ($item->getRowTotalAfterRedemptions()) {
