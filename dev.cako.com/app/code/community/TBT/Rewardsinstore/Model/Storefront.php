...
class TBT_Rewardsinstore_Model_Storefront extends TBT_Rewardsinstore_Model_Loyalty_Rewardsinstore 
{
    ...
    public function getFormattedAddress($format = 'oneline')
    {
        ...
        if ($format == 'oneline') {
            ...
            Mage::helper('rewardsinstore')->logError('Invalid format given for storefront address');
        }
    ...
    public function generateUrlCode()
    {
        ...
        $code = preg_replace("/[\s\/]+/", "-", trim(preg_replace("/[^-_a-z0-9\s\/]+/", "", strtolower($this->getCity()))));
    ...
    public function getActiveStorefronts() 
    {
