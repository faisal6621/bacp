...
class TBT_Rewardsinstore_Block_Webapp_Login extends Mage_Core_Block_Template
{
    ...
    protected function getStorefront()
    {
        ...
        if (count($storefronts) == 1) {
            $storefrontCode = $storefronts->getFirstItem()->getCode();
        } else if (!$storefrontCode = $this->getSelectedStorefront()) {
            $storefrontCode = $this->getDefaultStorefront();
        }
        return $storefrontCode ? "'{$storefrontCode}'" : "null";
    }
    ...
    protected function getDefaultStorefront()
    {
        $storefrontCode = Mage::getSingleton('core/session')->getCookie()->get('rewardsinstore-storefront');
        return $storefrontCode ? $storefrontCode : null;
    }
