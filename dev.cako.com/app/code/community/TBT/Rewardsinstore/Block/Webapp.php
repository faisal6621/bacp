...
class TBT_Rewardsinstore_Block_Webapp extends Mage_Core_Block_Template
{
    ...
    protected function getLaunchUri()
    {
        ...
        $paramKeys = array_keys($this->getRequest()->getParams());
        $params = count($paramKeys) ? array('code' => $paramKeys[0]) : array();

        $key = Mage::getSingleton('core/session')->getCookie()->get('rewardsinstore-key');
        if ($key) {
            $params['key'] = $key;
            return Mage::getModel('adminhtml/url')->getUrl('rewardsinstoreadmin/webapp_ajax/main', $params);
        }

        // If there's a parameter, use its key as our storefront code.
        return Mage::getModel('adminhtml/url')->getUrl('rewardsinstore/index/login', $params);
    }
