...
class TBT_Rewardsinstore_Manage_StorefrontController extends TBT_Rewardsinstore_LoyaltyController
{
    ...
    {
        return Mage::getSingleton('admin/session')->isAllowed('rewards/instore/storefront');
    }
    ...
    public function saveAction()
    {
        if ($this->getRequest()->isPost() && $postData = $this->getRequest()->getPost()) {
            ...
            try {
                $website = Mage::app()->getWebsite($postData['storefront']['website_id']);
                if (!$website->getDefaultStore()) {
                    $session->addError($this->__('"%s" does not have a default store view, which is required.', $website->getName()));
                ...
                Mage::helper('rewardsinstore')->logException($e);
