...
class TBT_Rewardsinstore_Manage_Earning_CartController extends TBT_Rewardsinstore_LoyaltyController
{
    ...
    {
        if (count(Mage::getResourceModel('rewardsinstore/storefront_collection')) < 1) {
            if (count(Mage::getResourceModel('rewardsinstore/cartrule_collection')) < 1) {
                if (!$this->_forwardToBillboard('rewardsinstore/billboard_nostorefront')) {
                    die ("You do not have any storefronts yet!  Please <a href='{$this->getUrl('rewardsinstoreadmin/manage_storefront')}'>create a storefront</a>.");
                }
                return $this;
            }
        }
    ...
    protected function _isAllowed ()
    {
        ...
        return Mage::getSingleton('admin/session')->isAllowed('rewards/rules/instoredistribution/instorecart');
    }
